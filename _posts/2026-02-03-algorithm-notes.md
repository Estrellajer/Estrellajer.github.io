...
步骤总结：
Q与K转置做矩阵乘法 -- 除以√d_k缩放 -- (可选)mask -- softmax归一化 -- 与V做矩阵乘法
...
步骤总结：
Q/K/V线性投影 -- 重塑为(batch,seq,num_heads,head_dim) -- 转置为(batch,num_heads,seq,head_dim) -- 每头计算QK^T/√d_k -- (可选)mask -- softmax -- 与V相乘 -- 拼接多头并输出投影
...
步骤总结：
Q全头线性投影、K/V共享头线性投影(head_dim) -- Q分头、K/V保持单头 -- 每头Q与共享K^T做点积/√d_k -- (可选)mask -- softmax -- 与V相乘 -- 拼接多头并输出投影
...
repeat(1,1,n_rep,1,1): (B, num_kv_heads, n_rep, seq, head_dim)  每 kv 头重复 n_rep 次
...
步骤总结：
Q全头线性投影、K/V分组头线性投影(num_kv_heads×head_dim) -- Q分num_heads头、K/V分num_kv_heads头 -- repeat_kv将K/V扩至num_heads -- 点积/√d_k -- (可选)mask -- softmax -- 与V相乘 -- 拼接多头并输出投影

import torch
from torch import nn

class GroupedQueryAttention(nn.Module):
def **init**(self, hidden_size, num_heads, num_kv_heads=None):
super().**init**()
self.hidden_size = hidden_size
self.num_heads = num_heads
...
**gather 要点**：`log_probs[b,i,labels[b,i]]` 等价于 `gather(log_probs, dim=-1, index=labels.unsqueeze(-1))`。index 必须与 log_probs 同维，故 labels (B,S) → unsqueeze(-1) → (B,S,1)。

**gather 具体例子**：

```
log_probs: (1, 3, 5)  即 1 句 3 词，词表大小 5
  位置0: [-2.1, -0.5, -1.0, -3.0, -2.5]  # 5 个 token 的 log 概率
  位置1: [-1.2, -2.3, -0.8, -4.0, -1.5]
  位置2: [-3.0, -1.0, -0.3, -2.0, -2.2]

labels: (1, 3) = [[2, 0, 4]]  # 真实 token id：位置0 是 2，位置1 是 0，位置2 是 4

labels.unsqueeze(-1): (1, 3, 1) = [[[2],[0],[4]]]

gather(dim=-1): 沿 vocab 维，按 index 取值
  out[0,0,0] = log_probs[0,0,2] = -1.0   ← 位置0 真实 token 2 的 log p
  out[0,1,0] = log_probs[0,1,0] = -1.2   ← 位置1 真实 token 0 的 log p
  out[0,2,0] = log_probs[0,2,4] = -2.2   ← 位置2 真实 token 4 的 log p
...
""" # ① log_softmax: (B,S,V) → (B,S,V)，每行和为 log(1)=0
log_probs = F.log_softmax(logits, dim=-1)

    # ② gather: 沿 dim=-1 按 index 索引取值，见上方「gather 具体例子」
    per_token_log_probs = torch.gather(
        log_probs, dim=-1, index=labels.unsqueeze(-1)
    ).squeeze(-1)

    # ③ mask 只保留 response 位置，④ sum 得到标量（每样本一个）
    return (per_token_log_probs * mask).sum(dim=-1)  # → (batch,)

def dpo_loss(
policy_chosen_logps: torch.Tensor, # (batch,) 当前模型在 chosen 上的 log p
policy_rejected_logps: torch.Tensor, # (batch,) 当前模型在 rejected 上的 log p
ref_chosen_logps: torch.Tensor, # (batch,) 参考模型在 chosen 上的 log p
ref_rejected_logps: torch.Tensor, # (batch,) 参考模型在 rejected 上的 log p
beta: float = 0.1,
) -> torch.Tensor:
"""
...
rejected_log_ratios = policy_rejected_logps - ref_rejected_logps
    ...
    # ② logits = β·(chosen_ratio - rejected_ratio)，越大越好
    logits = beta * (chosen_log_ratios - rejected_log_ratios)

    # ③ -log σ(x) 等价于 log(1+exp(-x))，数值稳定
